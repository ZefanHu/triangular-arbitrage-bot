...
│   ├── test_slippage_and_config.py # 滑点与配置校验测试
│   ├── test_precision.py     # 精度截断函数单元测试
│   ├── test_leg_recovery.py  # 三腿失败回收集成测试
│   ├── test_trade_amount_flow.py # 交易数量与资产流转测试
│   └── test_environment_gate.py  # 环境入口校验测试
├── 🔌 okex/                 # OKX官方API库
├── 📝 main.py               # ⭐ 主程序入口
...
path1 = {"route": "USDT->BTC->ETH->USDT", "steps": [{"pair": "BTC-USDT", "action": "buy"}, {"pair": "ETH-BTC", "action": "buy"}, {"pair": "ETH-USDT", "action": "sell"}]}
path2 = {"route": "USDT->ETH->BTC->USDT", "steps": [{"pair": "ETH-USDT", "action": "buy"}, {"pair": "ETH-BTC", "action": "sell"}, {"pair": "BTC-USDT", "action": "sell"}]}

# 初始持有量配置（根据实际账户资金设置）
initial_usdt = 4800            # USDT初始数量
initial_usdc = 0               # USDC初始数量
initial_btc = 0.9              # BTC初始数量
initial_eth = 4.53             # ETH初始数量

# 交易手续费配置（支持差异化费率）
fee_rate = 0.001               # 默认手续费率（兜底配置）

# 按交易对的差异化手续费配置（根据OKX实际费率设置）
fee_rate_btc_usdt = 0.001      # BTC-USDT手续费
fee_rate_eth_usdt = 0.001      # ETH-USDT手续费
fee_rate_eth_btc = 0.001       # ETH-BTC手续费

# 交易执行参数
slippage_tolerance = 0.002     # 滑点容忍度 (0.2%，用于套利收益测算与执行下单价格)
...
- **schema 校验与 fail-fast**：`settings.ini` 会基于 schema 进行校验，未知 key 会直接报错并终止启动（避免静默忽略）。  
- **数值范围约束**：交易/风险参数存在最小值、最大值或互斥逻辑校验（如比例上限）。  
- **支持 inline comment**：数值后可使用 `#` 或 `;` 追加注释，解析时会自动剥离。  
- **动态 key 规则**：`trading` 支持 `path*` 与 `fee_rate_*` 等动态 key。  
- **弃用参数**：`price_adjustment` 已弃用；若仍在配置中出现，会被映射到 `slippage_tolerance` 并提示弃用警告。  
...
- `tests/test_misc.py`
- `tests/test_slippage_and_config.py`
- `tests/test_precision.py`
- `tests/test_leg_recovery.py`
- `tests/test_trade_amount_flow.py`
- `tests/test_environment_gate.py`
...
# 运行滑点与配置校验测试
python tests/test_slippage_and_config.py

# 运行精度截断/三腿回收/资产流转/环境入口测试
python tests/test_precision.py
python tests/test_leg_recovery.py
python tests/test_trade_amount_flow.py
python -m pytest tests/test_environment_gate.py
```

> `tests/test_run_core.py` 会调用 OKX API 并依赖网络与有效 API 凭据。
> `tests/test_slippage_and_config.py` 为本地配置与计算逻辑测试，不需要网络与凭据。
...
### ⚠️ 安全第一
- **强烈建议 / 最佳实践**: 首次运行请使用模拟环境（`flag = 1` in secrets.ini）。系统不会强制校验“首次运行必须模拟盘”，实际以 `secrets.ini` 的 `flag` 为准，请在启动前自行确认当前环境（模拟/实盘）并评估风险。
- **资金安全**: 实盘测试请使用少量资金
